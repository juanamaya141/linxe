...
/**
 * Provides a 'FirmaTerminosAdelantoNBlock' block.
 *
 * @Block(
 *  id = "firmaterminosadelanto_block",
 *  admin_label = @Translation("Firma Terminos Adelanto Nomina block"),
 * )
 */
class FirmaTerminosAdelantoNBlock extends BlockBase {
  ...
  public function build() {
    ...
    if($session->has('tipodocumento') && $session->has('numerodocumento'))
    {
      ...
      $infoObj->fecha_actual = date("Y-m-d H:i:s"); 
    ...
    return array(
    	...
    	'#name' => 'Firma Términos Adelanto de Nómina',
      '#infoObj' => $infoObj,
      '#attached' => [
          'library' => [
