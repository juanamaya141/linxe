...
Drupal.behaviors.LinxecreditBehavior = {
  attach: function (context, settings) {
    ...
    if (jQuery('body').hasClass('dashboard')) {
        if(m==0)
        {
            document_type = parseInt(drupalSettings.linxecredit.librarycontrato.document_type);
            document_number = parseInt(drupalSettings.linxecredit.librarycontrato.document_number);
            paso3();
    ...
    function callOTP()
    {
        ...
        if(enviandoOTP==false)
        {
            ...
            jQuery.ajax({ //Process the form using $.ajax()
                ...
                url       : '/servicios/lanzarotp', //Your form processing file URL
